...
description: |-
  ...
  触发场景: 查询指定数据库、列出数据库表、查看已配置连接。
  关键词: 查数据库、数据库查询、SQL 查询、DB query、查询 XX 库、列出数据库表、数据库连接列表。
agent_created: true
---
...
- 执行前自动 EXPLAIN 预估扫描行数
- 无 LIMIT 的 SELECT 自动追加 `LIMIT 200`
- 预估行数 > 50K 时显示醒目警告
- `--count` 只跑 COUNT(*) 不取数据
...
```bash
python scripts/query.py --env dev <db_alias> --show
python scripts/query.py --env prod <db_alias> --show
```
